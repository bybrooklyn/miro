...
**Not yet done - the next concrete step:** none of Miro's own code (`apps/mirod`, `packages/sdk`) has been switched over yet. `apps/mirod/package.json` and `packages/sdk/package.json` still depend on the OLD `@earendil-works/pi-ai`/`pi-agent-core` (`^0.84.4`) - a real ~18-major-version API jump from what's now vendored. The ~15 import sites (`apps/mirod/src/agent/*`, `apps/mirod/src/extensions/*`, `packages/sdk/src/index.ts`) need to be re-pointed at the new `@miro/model-client`/`@miro/agent-core`/`@miro/schema-engine` packages and re-verified against the real 18.1.8 API shapes (`streamSimple`, `Agent`, `AgentTool`, `Type`, `builtinModels` -> `pi-catalog`'s `getBundledModels`/`createModelManager`) - this is the step that makes the vendor actually load-bearing rather than just present in the tree.
