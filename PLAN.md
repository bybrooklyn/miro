...
### 5.27 Capability layer, slice 3: extension-declared implementations, providers in context (2026-09-05)

Same branch (PR #6, now slices 1-3), commit `fd6289d`. §5.14's "extensions add new implementations of existing capabilities with no core change", scoped to the two capabilities that exist.

- **SDK:** `ExtensionModule.implements?: [{ capability: "web.search" | "web.fetch", entry }]` - the named `tool` entry takes the capability's request and returns its canonical response. The learn prompt says when to declare it (the app IS a search engine or a page reader) and what shape to return.
- **Validation:** the host's `list_tools` now reports the module's declarations (`listModule`); for each, the entry must be a tool of the extension, the capability must be implementable, and a live probe with the capability's request (`{ query: "debian" }` / `{ url: baseUrl }`) must come back in the canonical shape (`checkCapabilityResult`, rule `capability`, a fix hint with a worked `implements` example). Recorded in the manifest.
- **Registration:** `capabilities/extensions.ts` builds `ext:<app>:<capability>` implementations whose `run` is a host call to the entry, normalized with provenance; registered wherever the extension's tools are built (agent build, hot-load after a promotion), availability read from the DB on every route so a disabled extension - a failed pin, a tripped breaker - drops out with no unregister. Policy: after the self-hosted node and before the public pool for search (an app the owner runs beats a stranger's node); before the direct fetch for fetch.
- **Providers the agent can see:** the context block gains "Research providers (web_search / web_fetch route through these, best first)" - one line per capability, the public pool collapsed to a count; the `capabilities` tool returns `capabilityProviders` with live health (score, requests, failures, last 429, remaining with its confidence tag).

**A real bug the new context test found:** the registry keyed implementations by id globally, so registering web.fetch's `ollama` silently replaced web.search's - Ollama cloud search could never have routed, key or no key. Implementations are per capability now, with a regression test.

**Live on the dev VM:** asked to learn "searxng" (the container Miro installed in §5.26), the learn agent generated an extension declaring `implements: [{ capability: "web.search", entry: "search" }]`, the validator's live probe accepted it, promoted in one attempt (75s). With the self-hosted setting removed for one turn, `web_search` was served by `ext:searxng:web.search` (first hit "Release Notes for Debian 13 (trixie)"), and the `capabilities` tool listed all five providers with the right availability flags; the setting was restored.

**Not done from the omp-review's slice-3 list, deliberately:** `pi-iso` isolation primitives and browser/computer control are not capability work and each is a unit of its own; nothing here depends on them. 358 pass / 0 fail / 14 skip; every package typechecks.

### 5.28 Full codebase audit: what changed, why, and what was left alone (2026-09-05)

The standing order after the capability slices: "do a full audit on the codebase and do any improvements you think would be good; document everything you do and why; avoid changing functionality unless it is beneficial to user and agent; do NOT ask any questions; if anything is ambiguous, skip it and document why." Branch `audit` (stacked on `capability-web-search`, PR #7 on #6), four commits: `f1372ba` (tool results), `09642e3` (daemon hardening), `6ab7637` (protocol, client, repo), and the host-session/tests commit after it.

**Method.** Three independent read-only audit passes covered the whole tree by area (agent loop + capabilities + memory + `index.ts`; operations + extensions + discovery + secrets + inventory; the SDK, protocol, ui-model, TUI, docs, build files and the vendored-package surface). Every finding was re-verified against the code before anything changed - the ones marked "verified by running" in the passes had already been reproduced with real servers, real subprocesses or real `bun -e` probes, never a mock. Roughly 85 findings; the ones acted on are below, grouped by what they protect, each with the reason it was worth a behaviour change. The tests added with them are the house style: real `:memory:` databases, real fixture HTTP servers, real temp files and symlinks.

**Security - things that let a model-controlled input reach past a boundary.**
- `web_fetch` followed every HTTP and client-side redirect unchecked, so a public page could 302 (or `<link rel=canonical>`) the daemon into `127.0.0.1:8096` and return an unauthenticated local service's body to the model - reproduced with two local servers. Every hop is now checked against the private-address guard and redirects are manual (`redirect: "manual"`, target inspected before it is fetched); hop cap 8. Test: a 302 into `127.0.0.1:9` and a canonical link into `10.0.0.5` are refused, the jellyfin.org redirect stub still resolves.
- `isLocalOrPrivateUrl` tested the IPv6 ULA/link-local prefixes against ANY hostname: `fdsomething.com` and `fc2.com` were "private", so a read-class `curl` to them left the LAN unconfirmed. The prefixes apply to IPv6 literals only; IPv4-mapped literals (`[::ffff:127.0.0.1]`, which `new URL` renders as `[::ffff:7f00:1]`) and `0.0.0.0` are private now.
- `isSensitivePath` was a string check; a symlink at an innocent path into `~/.ssh` or `/var/lib/miro` passed `read_file`, `ctx.readFile` and every file kind (`ln` is a confirmable mutate, so an agent can plant one). It now judges the string AND `realTarget()` (moved into `classify.ts`). `shell.command`'s declared write roots are judged the same way - a bind mount follows the symlink, so `/srv/app/data -> /etc` would have been approved as `/srv/app/data` and mounted as `/etc`.
- `shell.command` write scope: `writes: ["/etc"]` was a plain mutate with `shadow`, `sudoers` and the SSH host keys bind-mounted read-write, while the narrower `/etc/ssh` was correctly lifeline. A root that is or contains Miro's state or secret material (`/`, `/etc`, `/var`, `/var/lib`, `/root`, `/home`, `/home/<u>`, `/proc`) is refused with the fix ("name the exact directory"); `/usr`, `/bin`, `/sbin`, `/lib`, `/run`, `/etc/systemd`, `/var/lib/docker` escalate to lifeline; a non-existent root named like a file is refused (the sandbox `mkdir -p`s it into a directory and the command's write fails); `badWriteScope` refuses `/proc`, `/sys`, `/dev` and block devices. The snapshot cap is now known at describe time: a plan for roots past 64 MB says nothing is snapshotted and is irreversible without an undo command - it used to promise "restored from snapshot" and restore nothing.
- The systemd kinds carried a five-unit copy of the lifeline list, so `service_control stop docker.service` (or `nftables`, `tailscaled`, `containerd`, `mirod`) ran unattended as a plain mutate. `LIFELINE_UNITS` in `classify.ts` is the one list for both kinds; stopping or restarting `mirod` itself is refused - the daemon dies mid-operation and can neither verify nor roll back.
- Extensions: the `app` name from `app_learn` reached `mkdir`/`rename`/`rmSync` as root unvalidated (`"../../etc/ssh"` resolved to a real directory promotion would have renamed aside). `APP_NAME` (`^[a-z0-9][a-z0-9_-]{0,63}$`) is enforced in every path builder and at the learn entry. The import scan allowed `../` (walking into the daemon's own source) and could not see a computed `import()` - same-directory only now, and a non-literal specifier is refused. `@miro/sdk`'s `HttpClient` resolved an absolute or protocol-relative `path` off-origin with the app's real credential in its fixed headers (the exact hole the write path already guarded); it stays on the app's origin, never follows a redirect, and caps the body. `validateEntry` enforces "exactly one of read/bind/code" (an entry with both ran the read and silently dropped the model's code).
- `http.mutation` re-checks every URL after secret substitution, at request time (describe judges the literal; a placeholder in the authority does not even parse, so this is defence in depth). File plans redact the file's EXISTING lines (`current`, and the context/removal lines of the diff) - an app config's database password is not Miro's to persist with the plan and send to every client; the `+` side is the model's own input and stays legible for approval.
- Classifier holes, each a case in `classify.test.ts`: `env LD_PRELOAD=… cat f` (the wrapper skipped assignments unread - LD_*, DYLD_*, GIT_*, BASH_ENV, PATH, PYTHONPATH, NODE_OPTIONS … are refused, `env FOO=bar ls` stays read); `ssh -o ProxyCommand=…` (a local command read as the inner one's class); awk `print | "cmd"` and sed `w file` (writes with no `-i`); `&>` (tokenised as background-`&` + bare `>`, so `ls &> /tmp/x` was refused for the wrong reason and never classified); `docker run --privileged=true` / `--pid host` / `--volumes-from` (flag spellings); and a content reader aimed by an unresolved expansion (`cat $HOME/.ssh/id_rsa` demoted to a confirmed mutate that READ the file with the host filesystem visible - refused now, with "write the literal path").
- `redactSecretsInText` covers a quoted value whole (`"password": "two words"` used to leave ` words"`), flag-form credentials (`--password X`, `--token=X`; bare `-p` deliberately not - it is also a port flag), `curl -u user:pass`, userinfo in a URL, and never eats a `{{secret:ref}}` placeholder. The first redaction tests exist now.
- `container_logs`, `systemd_logs` and every other tool answer pass through one redacting `textResult` (`agent/tool-result.ts`); nine byte-identical private copies existed and eight had dropped the scrub. The Iroh pairing ticket - "anyone with this string gets full daemon access" - is no longer printed to the daemon log at every boot (a durable, shipped file in production); the NodeId is, and `/pair` shows the ticket. `fs_index` skips secret paths (a listing of `~/.ssh` is a map to what every other read path refuses to draw).

**Bugs - things that were wrong or hung.**
- No inventory subprocess had a timeout (`promisify(execFile)` with `timeout: undefined` has none): `docker ps` against a wedged socket hung `takeSnapshot` on every chat turn with `turnActive` latched, and `computeSeverity` inside the write lock could hang every future operation. `run()` defaults to 30s; long-running callers (tar, docker run) pass their own. `packages`, `df` and `docker inspect` degrade instead of throwing.
- The termination gate counted `applied_unverified` as a commit, so a turn of unconfirmed writes ended as "done" - the exact case the gate exists for. It counts only `committed` (+ test).
- One malformed client line threw out of the socket's data handler into the error callback, which cancelled every pending confirmation on the connection (Iroh: ended the session). Dropped with a warning now, like the client already did. `createLineBuffer` decoded each chunk separately, so a multi-byte character split across chunks became U+FFFD on both transports (10 of 53 split points of a "café ✓ 日本 🔑" line, brute-forced in the test); it stream-decodes and drops a line past 16 MB instead of growing without bound.
- Boot: a missing `getent` threw out of `spawnSync` after the socket was bound (daemon down; now degrades to root-only with a 5s timeout); a half-written `codex-auth-import.json` was a boot loop (guarded, left in place, logged); a corrupt or partial OAuth blob threw into every chat turn (reads as "not connected"; the CLI import requires access/refresh/expires).
- Engine: a `captureState` throw left the row in `capturing` and the client's operation unresolved until the next boot (resolved as rolled back, nothing touched); a lifeline timeout left its answer waiter registered forever (`cancelAnswer`); a torn row aborted reconciliation, the drift sweep, the extension health sweep and the trash index for every later row (per-row guards).
- Severity: the "recent incidents" term is gone from the before/after comparison. Incidents are written by background sweeps outside the write lock and within one operation the count can only rise - the term could only force a false "the server got worse" rollback of a verified change, never catch a real one.
- Prodtest keyed targets by kind, so a path Miro trashed and later rewrote was two "latest" checks and the delete's failed every sweep forever (a reinforced false incident). Keyed by target across kinds (+ test).
- `searxng.install` destroyed a pre-existing `miro-searxng` container and declared a rollback it could not give; it names the replacement as irreversible with a warning.
- Extension host: `listModule` leaked its subprocess on every error path; a non-JSON stdout line killed the reader loop (the session stayed "alive" and every later call timed out); a `console.log` in generated code wrote into the framing channel; a timed-out session stayed in the map. One `teardown()` now: shutdown, map entry gone, pending calls rejected, kill after 3s if the process has not exited; `console.log`/`console.info` go to stderr in the host; a timeout tears the session down. The validation session (keyed on the staging dir) is dropped and a failed staging dir discarded, as the comments claimed; the idle health probe no longer counts toward maturity (ten timer probes used to make an extension "trusted" with zero real use).
- `secrets.ts` names a truncated key file at boot instead of "Invalid key length" from every call for the process's life. The capabilities tool advertised unsanitised `ext_<app>_<tool>` names; the extension-backed capability implementations ignored the router's abort signal (a hung entry hung the route); two public SearXNG nodes on one host collapsed into one implementation id; `capabilityContextLines` replaced only the first dot; the router's fan-out never resolved on an empty candidate list.
- Client: a daemon restart left the TUI on "working… (one request at a time)" with no input and offered answers nobody was listening for - `status: connecting` ends the turn and forgets the daemon's open questions; the Iroh transport never reported a dropped stream (stale "healthy" forever) - it reconnects like the unix path; `send()` reports whether anything was written and the UI advances only when it was; `a`/`y` fell back to the first option of ANY question (`y` picked "Anthropic" on the provider chooser and "cheapest" where "best" was recommended) - a letter answers only a question with that option, and `/provider` has a Cancel; `resolveSocketPath` ignored `MIRO_DIR` (AGENTS.md promised it worked); status is `degraded` when no provider is connected (the client already rendered it); a typo'd slash command is answered locally instead of spending a model turn; chat sends the trimmed text it shows.

**Robustness and cost.** Every untrusted HTTP body is read to a cap off the wire (`fetch-body.ts`: pages 1 MB, search answers 1 MB, `http_get` 64 KB, the searx.space list 16 MB) instead of buffered whole then sliced; the public pool refresh is capped at 120 candidates and 90s; Ollama is re-probed when nothing at all is connected (so an Ollama started after the daemon is usable without a restart); `read_file` clamps `maxBytes`/`offset` (a negative value was an opaque `RangeError`); the last chat model is per connection; `file.delete` sizes once with a 1 GB cap (an uncapped walk of `/var/lib/docker` ran for minutes before the owner was asked); the diff table is 9 MB at most (1500 lines; was 64 MB at 4000); the extension validator loads no ambient `@types` (23s → 17s per attempt, measured; the DOM lib stays - the schema engine's `File`/`FormDataEntryValue` need it); the reference-extension typecheck test has an honest 60s timeout (it ran 8-10s cold and the suite was red on a cold cache).

**Docs, dead code, tests.** The refusal table and the classifier's alternatives no longer name a "reboot operation" that does not exist (the agent was being directed to a tool it did not have); the validator comment says only no-argument bindings are dry-run; the host-entry header states the containment boundary honestly (imports are allowlisted; a `code` entry has the process's globals - what bounds it is the unprivileged user, the scrubbed env, the pin and the classifier behind `ctx.exec`); the read-tools comment no longer claims workers can see them; reflection may not write the `capability` category (a one-sentence row would corrupt the JSON document the capabilities tool parses). Dead: the SDK's `createFakeExec`/`createFakeReadFile`/`ExtensionTool`/`ReadBinding.method`, the client's `smoke.ts` (an unrun second renderer), `LIFELINE_ADJACENT`, `alpnBytes` (renamed `utf8Bytes` - it carried protocol lines at four sites), the orphaned `ExtensionModule` doc comment, the footer's "esc interrupt" (no such message exists). Repo: AGENTS.md lists the vendored packages and the `just` recipes as the commands (it described four packages and no typecheck script; `just check` covers 13); the SDK surface it names is the one that exists; `justfile`'s renv comment no longer protects a registry that is gone; the root package is `miro-monorepo` (it collided with `apps/miro`); `bun run dev` stops the daemon when the TUI exits; `up.sh` names a missing qemu or probe helper instead of a three-minute timeout; `.claude/*.lock` and `auth.json` are ignored and the committed session lock untracked; PLAN.md's "this is not a git repository" and pi-ai CLI bullets are gone and Part 5's heading is restored ahead of §5.11 (seventeen design records had been filed under "gotchas"). Tests added: the SSRF shapes and `fetchWeb` truncation, the URL guard's v6/mapped/`fd*` cases, `applied_unverified` at the gate, the corrupt-blob and partial-import paths, write-scope refusals with a real symlink, the lifeline units and self-refusal, app-name refusal in every path builder, the import scan's three new refusals, entry exclusivity, the SDK origin guard against a real server, the classifier rows above and the first redaction suite, the cross-kind Prodtest key, the router's fan-out width and deadline through `route()`, the UTF-8 split and the line cap, the client's connecting/keymap/footer/slash-command behaviour, `resetCapabilities()` so the one test that configures the module singleton stops leaking into every later file.

**Skipped, and why (the "ambiguous → skip and document" rule).**
- *Replacing the extension host's globals* (`fetch`, `Bun.spawn`, `process.env`) with guarded versions so a `code` entry cannot egress: a real containment upgrade, but a design change with a compatibility question (the SDK's own client uses `fetch`). Documented as the boundary's named upgrade path instead.
- *A file written at the filesystem root* (`file_write /motd`) is refused as "scope is the root" - the kinds declare the parent directory; letting `/` through only for this case weakens the selector-sanity rule. Rare; left, noted.
- *`miro setup` (the SSH bootstrap screen) is unreachable from the shipped binary* and its own copy tells the user to run it. Whether the SSH-bootstrap flow is still part of the product after the root-service layout is a product decision - dispatching it and deleting it are both one-line changes with different meanings. Left as is; `reachability.ts` stays because `up.sh` uses it.
- *`export` on internal-only symbols* (about 35 across capabilities/agent): stripping them is churn with no runtime effect and a future import risk; left.
- *`bumpHelpful`/`bumpHarmful` and `superseded_by` have no production writer*: tracked debt per §5.18, not an oversight; left, noted here so it does not rot unseen.
- *An unreachable `catch` around `send()` in the codex-login notice path*: Bun's unix `write` after the peer closes returns -1 rather than throwing, so the branch never runs and the notice is simply dropped - harmless, left.
- *`ollama.test.ts` passes vacuously without a local Ollama*, and one memory test duplicates two others: both documented in place already; left.
- *`fs-snapshot.ts` has no caller*: deliberately detection-only (§5.18); left with its note. *`store.getOperation`* is a store API used by tests; left.
- *`packages/native`'s `napi.binaryName` is still `pi_natives`*: it matches the artifact name the fallback looks for on disk; renaming it is a build-pipeline change for an addon Miro does not build. *`agent-wire` lacks `exports`/`engines`*: cosmetic, transitive only.
- *The `-p` flag in redaction*: `mysql -phunter2` vs `ssh -p 22` vs `docker -p 8080:80` - ambiguous; the long forms are covered and the same value in output is caught by the other shapes.
- *`recentIncidentCount`* was removed from the severity comparison rather than snapshotted once, because a term that can only rise within an operation has no regression signal to offer at all.

**Live verification (dev VM, root daemon, real systemd, real docker, real internet).** Synced, restarted; boot log shows `mirod also reachable via Iroh - node ed2d9e…; /pair shows the pairing ticket` and no ticket. A 22-check script against the real kinds and the real read tool: `stop docker.service` is lifeline and not auto-approved; `stop mirod.service` / `restart mirod` refused; `start jellyfin.service` mutate; `writes: ["/etc"]` and `writes: [symlink → /var/lib/miro]` refused, `/usr` lifeline, `/etc/nginx` mutate, a non-existent `config.ini` root refused; `read_file` of `/tmp/audit-leak.key → /var/lib/miro/secret.key` refused, a plain file reads; the URL guard's `fdsomething.com`/`[::ffff:127.0.0.1]` cases; `env LD_PRELOAD` forbidden, `ls &> /tmp/x` mutate, `cat $HOME/.ssh/id_rsa` forbidden, sed `w` mutate; `searxng.install` sees the running `miro-searxng` and marks the plan irreversible; `web_fetch` of an IPv4-mapped loopback refused before routing, and jellyfin.org's hardware-acceleration page (the client-side redirect stub from §5.26) still resolves to 6234 chars of content through the manual-redirect path. Two chat turns through the restarted daemon (Codex): "what version of jellyfin is running here, and what is in your trash?" answered from the extension's `get_server_info` through the reworked host session plus the new `trash_list` (10.11.11; 10 trashed items listed); "how many jellyfin users exist, and which libraries are configured?" answered from `list_users` and the libraries entry. Not live-verified: the TUI changes (headless VM; covered by ui-model's tests against real event sequences) and the Ollama implementations (still no cloud key).

**Gotchas found on the way.** `timeout(1)` does not exist on macOS - a `timeout 120 bun test …` in a pipeline silently ran nothing. The documented VM restart (`sudo -b sh -c '… nohup … > log 2>&1 < /dev/null'`) holds the ssh session open because the `sh` parent keeps the session's stdout; `sudo sh -c '… nohup … &'` returns at once (AGENTS.md updated). `node_modules` vanished mid-session on the dev Mac twice more (something outside this session removes it; `just renv` restores in seconds - another session's cache testing, per the earlier note). Bun's `bun test <dir>` from a package cwd matches nothing and exits 0 - always from the repo root.

**State:** 379 pass / 0 fail / 14 skip; every package typechecks (`just check`, 13/13). Next: PR #7, then Stage D's app set on the hardened base.

### 5.29 Stage D slice 2: the golden-proof runs - media acquisition around Jellyfin (2026-09-05)

Branch `stage-d-media` (stacked on `audit`, PR #8 on #7). §5.5's slice 2: one outcome message, the agent architects and builds the acquisition stack (Prowlarr, Radarr, Sonarr, qBittorrent) around the existing Jellyfin, recursively learning each app, wiring them together, verifying the path. Torrents, no VPN (no VPN credential exists on the dev VM, so the VPN/kill-switch verification half of slice 2 is not exercised - stated, not hidden), no public indexers (the owner adds those). Every run: the root daemon on the dev VM, Codex (gpt-5.6-luna), the headless driver playing the owner (approves plans and operations, answers intent with torrents/no-VPN defaults, gives no secrets), the four linuxserver images pulled ahead so a 1.2 Mbit/s SLIRP pull is not mistaken for an operation timeout - the only hand-prepared state; everything else the agent did, and nothing on the VM was hand-edited between runs.

**Run #1 (cold, 18 min) - partial, honest.** Inspect-first was exact: containers, mounts, Jellyfin's libraries and their host paths, the network, port 8080 already Gotify's. The system plan (approved once) was the plan a self-hoster would write: reuse Jellyfin and its libraries, install the four apps with persistent config under `/home/miro/arr-config/<app>` and one shared `/home/miro/media → /data` mount, qBittorrent on 8081 because of Gotify, Prowlarr with zero indexers by request, verification steps end to end. It deployed all four, created `/home/miro/media/downloads`, learned and promoted all four extensions (Prowlarr 5 tools/2 ops, Radarr 8/3, Sonarr 7/3, qBittorrent 4/3), recorded four operational models, then stopped at the three-failed-plans cap with a reply that named exactly what was done and what blocked it. Independent check (`verify-media-stack.ts`: each app's own `config.xml` API key, its API asked directly): 19/24 - containers, persistence, mounts, keys and versions right; root folders, download clients and Prowlarr's applications not configured.

Findings from run #1, each fixed at the root (commit `a099af6`):
1. **Learn before install.** The agent fired four parallel `app_learn` calls for apps it had not installed; each session inspected, found nothing, researched anyway, and burned its write budget on a live probe against nothing (three sessions, ~5 minutes). The Prowlarr session then installed the app itself (asking first) to have something to learn from - inside a learn session, outside the system plan. Root cause: the validator's live probe and dead-app check REQUIRE a running instance, so learning an absent app can never promote. `app_learn` now returns at once when discovery finds no matching container or unit - "install and start it first, as an operation inside your system plan, then call app_learn again" (a hint with a URL still learns).
2. **A credential the machine produced could not be used.** linuxserver's qBittorrent prints a temporary WebUI password to its container log at first start. `container_logs` and `read_file` redact it (correctly - secrets never enter model context), so the agent could not use it and asked the owner for it three times - a value the owner does not have either. New tool `credential_capture { ref, from: {container|unit|path}, pattern }`: one regex capture group, the match goes into the secret store under `extension.<app>.<name>`, the model gets the ref and the matched line redacted. Same rule as `storeResponseField`, extended to logs and files. `storeResponseField.field` also accepts `header:<name>` and `cookie:<name>`: qBittorrent's login answers with a `SID` cookie and there was no way to keep a session cookie by reference either. The base prompt and the learn prompt say so: a first-start password, a config-file key, a login's cookie are captured, never asked for.
3. **Ownership.** Radarr (PUID 1000) rejected `/data/Movies` as a root folder - the host directories were root-owned. The agent planned the chown but had reached the cap. The EXECUTE step now says to check a host directory's owner against the container's user and fix it as an operation before pointing the app at it.
4. **`writes: ["/var/lib/docker"]`** on every `docker run` made each one a lifeline change with a reachability handshake (the audit's write-scope rules, working as designed on an over-declaration). The `writes` description and the prompt now say: a container's real writes are its bind mounts and the docker socket.

**Run #2 (continuation, 11.5 min, before the capture fix reached the VM) - partial, honest.** Zero secret prompts (finding 2's guidance held), ownership of `Movies`/`TV` fixed to 1000:1000, the shared network and mounts re-confirmed, still blocked on qBittorrent's login - and `credential_capture` refused every call as ambiguous: the model fills the unused optional sources with `null`, which the tool counted as a second source. Fixed (null, "" and a JSON-string `from` are absent), with a test. The three-rollback cap then blocked the root-folder and download-client writes that had nothing to do with qBittorrent.

**Run #3 (continuation, capture fix live) - closer, and the next finding.** `credential_capture` worked at +13s. The system plan came at +103s, the Radarr and Sonarr root folders were posted, and then both download-client adds failed their connection test (400) and the qBittorrent login answered 401 - the tool had taken the FIRST matching line, the password from before run #2's `docker restart`; qBittorrent mints a new one on every start and the log had several. Three plans later the cap ended the turn's writes and the agent drifted into a re-learn of qBittorrent that could not pass its live probe with a stale credential; the run was ended by hand at +266s once that was clear. Fixed (commit `de1a7a5`): the NEWEST matching line wins (a log appends), said so in the tool's description, tested with two starts' worth of lines.

**Run #4 (continuation, newest-match capture, cap 5; 4 min 10 s) - the wiring done.** Captured the current temporary password at +20s, planned at +48s, then: two logins through the host port mapping got 401 (qBittorrent's host-header check against `localhost:8081`), a login on the container's own `media-stack` address worked, the permanent password from the store was set via `/api/v2/app/setPreferences` and verified by logging in with it; qBittorrent added as the download client in Radarr and Sonarr at `qbittorrent:8080` (the first attempt of each 400'd on the connection test, the second committed); Radarr and Sonarr registered as Prowlarr applications and synced; every root folder accessible; a Jellyfin library refresh got 401 (the stored session token from an earlier day is stale) and the reply said so. 7 rollbacks, 2 applied-unverified (logins with no undo), 7 commits, 0 secret prompts; the cap of five was reached on the very last, cosmetic step. The reply listed each app's state and the one open item. **Independent check: 24/24** - Prowlarr has Radarr and Sonarr as applications and zero indexers; Radarr's root is `/data/Movies` and Sonarr's `/data/TV`, both with qBittorrent as an enabled download client; all three acquisition containers and Jellyfin share `/home/miro/media`; every container persistent and running. The agent recorded a composed `media_acquisition` capability: summary, the six components with versions, the data flow (`Radarr -> qbittorrent:8080 -> /data/downloads and /data/Movies`, the Jellyfin mapping), credentials by reference, and five verification checks.

**Retention, checked with a fresh turn** ("is my media acquisition stack healthy right now? one paragraph on how it is wired, and anything that needs attention."): within nine seconds the agent ran twenty of its learned tools in parallel (health, download clients, root folders, Prowlarr's applications and indexers, Jellyfin's libraries, the network) - no research, no re-inspection from scratch - then one qBittorrent tool answered 403, the inline repair loop fixed the extension mid-turn (`repaired: true`), and the reply was right: running, the four apps and Jellyfin on the shared network with the Arr apps reaching qBittorrent, "not functionally healthy" only because there are no indexers, by request. That is §5.1's "a month later it is one tool call", on the second day.

**On the three-failed-plans cap.** Three turns in a row ended on it while one real problem (qBittorrent's login) was being worked from different angles and sound, unrelated steps (root folders, Prowlarr registration) were still queued - the cap stopped progress, not thrash, and each stop cost a human turn to reset. Strong models now get five distinct failed plans a turn (weak keep two; an identical plan is refused regardless of count). Run #4 reached five only on its last, cosmetic step. Kept as a cap, not removed: it is what turned every one of these runs into an honest partial report instead of a runaway.

**Live-only findings, all of them.** Nothing here was visible to the test suite or the typechecker: the learn-before-install ordering, a redaction rule doing its job against the one value that had to flow, `null` for an unused optional field, first-match versus newest-match on an appending log, a host-header check on a port mapping, root-owned media directories against PUID 1000. Each is now a test where a test can hold it (the capture tool's cases, the cookie/header capture against a real server) and a prompt line where it cannot.

**Not done, by scope.** The VPN half of slice 2 (a gluetun-style tunnel, the kill-switch/DNS/exit-IP verification of §5.2 F) needs a VPN credential nobody on the dev VM has; packet capture never became necessary - the four apps document their APIs. A `download_movie` follow-up needs an indexer.

**The stale Jellyfin token, one turn later** ("refresh both jellyfin libraries now …"): the refresh got 401, the agent re-authenticated as the stored `admin` with the credentials on file (`{{secret:extension.jellyfin.admin_user}}` / `admin_password` in the body, the fresh `AccessToken` kept by `storeResponseField` - the second attempt, after the 10.11 `Authorization` header shape), stored the verified token over the stale one, and the refresh committed (204); it then read Jellyfin's scheduled tasks and confirmed the scan had run. On the way the retry ledger refused a retry of the exact plan that had just failed ("this exact plan already failed this turn"), which pushed the agent to a different one. 84 seconds, one approval per write, nothing asked of the owner. The design's answer to a dead credential is the agent's own re-authentication from stored refs, and it worked without a special case.

**State:** 381 pass / 0 fail / 14 skip; every package typechecks (`just check`, 13/13). PRs #3 → #4 → #5 → #6 → #7 → #8 are stacked and await review. What remains of Stage D is the VPN half of slice 2, which needs a credential only the owner has; Stage E (the capstone) is unstarted.

### 5.30 The systemd unit and the reboot operation (2026-09-05)

Branch `reboot-recovery` (stacked on `stage-d-media`, PR #9 on #8). The oldest open item on the Stage B row - "reboot recovery not started" since slice 1 - and the substrate §5.16's self-update auto-heal was designed to reuse. §5.18 marked it as the one engine-assurance item needing "a real design decision about the pending-bless marker format and systemd integration". Grilled with the owner, then built and live-verified on the dev VM through two real guest reboots.

**The design, decided.** A reboot is a confirmed, lifeline-class, irreversible operation (`system_reboot` → `system.reboot` kind, `operations/kinds/reboot.ts`). Its own apply() ends the daemon, so verify() can never run inside `runOperation`. The marker is the operation row itself: the `applying` row and its persisted `captured_state`, no marker file and no new table (the row is already a durable WAL commit - `setCapturedAndApplying` - before apply). `OperationKind` gains an optional `reconcile(params, captured)` hook; at the next boot `reconcileOperations` hands it a row found in `applying` INSTEAD of running verify/rollback, consulted right after the plan parse and BEFORE the irreversible branch (which would commit it blind) and the lifeline branch (which would roll it back, E2). The hook proves the reboot happened by the kernel boot id (`/proc/sys/kernel/random/boot_id`, exact and clock-independent - a no-RTC board can boot with a lagging clock and an uptime rule would misjudge it), settles, judges, and its message reaches the owner as a one-shot `notice` on the first client to connect after boot (that connection is the reachability proof a reboot has no timed handshake for; the report is cleared on delivery). §5.16's self-update kind reuses the same hook and the unit's stable ExecStart path.

**What comes back is fixed before the reboot, inside the one approval.** captureState inspects every running container; one whose effective restart policy is `no` gets `docker update --restart unless-stopped` in apply() before `systemctl reboot`, unless it is compose-managed and its file sets an explicit `restart:` key (any value, respected; `composeRestartKey` reads it with `Bun.YAML.parse`). A compose container with no key gets the runtime update and the report names the file to add it to. `runPrivileged` (new, in `inventory/exec.ts`) replaced every `run("sudo", ...)` site: root runs the command as-is, since the hardened unit's `NoNewPrivileges=yes` makes sudo refuse even for root.

**The hardened unit, flag by flag (`apps/mirod/mirod.service`, the daemon's first).** mirod had no unit anywhere; on the VM it ran by nohup and died on reboot, so a reboot could never reconcile. The unit is `Type=notify` (sd_notify `--ready` once the socket is bound plus a `WATCHDOG=1` heartbeat, via the `systemd-notify` binary since Bun has no AF_UNIX datagram client, guarded on `NOTIFY_SOCKET` so nohup dev runs keep working). Every §5.15 D flag was enabled on the VM and kept only if the existing live proofs held; each dropped flag stays listed in the unit with the operation it blocks. `tools/dev-vm/install-mirod.sh` installs it via a wrapper at `/usr/local/bin/mirod` that runs the source tree (production ships the binary there). `systemd-analyze security`: **9.6 UNSAFE → 6.9 MEDIUM.**

Kept (verified live): `NoNewPrivileges` (with the root-aware exec above), `MemoryDenyWriteExecute=yes` (Bun's JavaScriptCore ran a full model turn, tool calls and a sandboxed write under it - the one true unknown, and it held), `RestrictNamespaces=cgroup ipc mnt net pid user uts` (everything but `time`: bubblewrap as root and Chromium as the extension-host user both need it), `SystemCallFilter=@system-service @mount syslog` (`@mount` for bubblewrap; `syslog` because `dmesg` is an allowed read and `@system-service` excludes it, otherwise a SIGSYS "Bad system call"), `RestrictAddressFamilies=... AF_NETLINK AF_PACKET` (`ip`/`ss` and glibc `getaddrinfo`; `net_capture`'s tshark), `ProtectProc=invisible`, `ProtectControlGroups`, `ProtectClock`, `ProtectHostname`, `LockPersonality`, `RestrictRealtime`, `UMask=0022`, `WatchdogSec=60` (a `SIGSTOP`'d daemon was killed and restarted on schedule).

Dropped, each with the probe that showed it: `ProtectSystem` (any level makes `/usr` read-only and `apt` runs in bubblewrap inside this mount namespace), `ProtectHome` (the media stack and the extension host's home under `/home`), `ProcSubset=pid` (bubblewrap reads `/proc/sys/kernel/overflowuid` at start, so every sandboxed command fails - a stronger reason than the CPU/mem inventory one predicted), `ProtectKernelTunables` (sysctl tuning is a real operation), `ProtectKernelModules` (`/usr/lib/modules`, a kernel upgrade), `ProtectKernelLogs`, `PrivateTmp` (a `/tmp` write invisible to the owner and to `docker -v`), `PrivateDevices` (`/dev/dri`/`nvidia-smi`; not provable on the GPU-less VM, dropped on the code), `RestrictSUIDSGID` (`dpkg` sets setuid on `passwd`), `UMask=0077` (a `file_write` without a mode was unreadable by a PUID container). `ExecStartPre` self-update swap, `.socket` activation and `CapabilityBoundingSet` were left for later.

**The live proof.** Two real guest reboots through the headless driver (`chat-driver.ts` plays the owner: approves the plan, its socket dies with the reboot). Run #1 came back committed only after a correction (below); the five media containers (jellyfin, sonarr, radarr, prowlarr, qbittorrent), all `restart=no`, came back running with `unless-stopped` on their own - no hand `docker start jellyfin`, the exact chore AGENTS.md carried for months, gone. Run #2 (jellyfin reset to `no`): boot id changed `ecdffb3d → e2e6f938`, mirod reached active ~11s after boot, the row committed, the report `Rebooted - …, verified: 6 containers back and no failed units, severity 0->0` was delivered to the first connection as an info notice and cleared, the second connection got nothing (one-shot). A seeded did-not-reboot row (with `irreversible` and `lifeline` in its plan) went through the kind's own reconcile on a plain `systemctl restart`, proving the hook precedes both branches, and rolled back with "the server did not reboot (Miro restarted, the box did not)". Incidents recorded for all three.

**Two findings the reboot itself surfaced (fixed at the root, both live-only).**
1. **The unit-set diff was noise.** The first design captured every active `.service` and required each back; at early boot that flagged mirod itself (mid-reconcile, before it is `active`), oneshots that had already exited, and dbus/socket-activated units (polkit, upower) not yet triggered - all false "missing", and the settle poll ran its full two minutes because they never "came back". The honest signal for a lost unit is a FAILED unit, which the severity oracle already counts; a clean-inactive on-demand unit is not a reboot failure. The verdict now keys only on the containers (the explicit target) and the severity number, and the poll breaks in seconds. `systemctl is-system-running --wait` was also removed from the settle - reconcile runs before the socket binds, so the daemon is an unfinished unit systemd's own startup waits on, and `--wait` would deadlock against mirod's start.
2. **`file.write` treated `mode: null` as a mode.** The hardened unit's UMask probe wrote two files with `mode: null` (what a strict-schema provider sends for an omitted optional) and they came out `----------`: `chmod(path, null)` is `chmod 0`. Now `!= null`, with a test. Unrelated to the reboot, found because the probe exercised the write path under the new UMask.
