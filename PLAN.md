...
**Not done, by scope.** The VPN half of slice 2 (a gluetun-style tunnel, the kill-switch/DNS/exit-IP verification of §5.2 F) needs a VPN credential nobody on the dev VM has; packet capture never became necessary - the four apps document their APIs. A `download_movie` follow-up needs an indexer. The stale Jellyfin session token should be re-minted by the jellyfin extension's own repair path (the stored admin credentials are on file) - a candidate for the next turn's finding.
