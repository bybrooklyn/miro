...
import { type } from "@miro/schema-engine";
...
class CodexWebSocketConnection {
	...
	async connect(signal?: AbortSignal): Promise<void> {
		...
		socket.onmessage = event => {
			// Vendoring note: this bun-types version declares Bun.WebSocket's onmessage parameter as
			// `typeof MessageEvent` (the class), not an instance - a real bun-types bug. Cast locally
			// rather than annotate the parameter, which would mismatch the (buggy) assignment target.
			...
			try {
				const text = typeof messageEvent.data === "string" ? messageEvent.data : Buffer.from(messageEvent.data).toString("utf-8");
				if (!text) return;
...
async function openCodexSseEventStream(
	...
	const send = (requestBody: string | Uint8Array): Promise<Response> =>
		fetchWithRetry(url, {
			method: "POST",
			headers,
			body: requestBody as BodyInit, // Vendoring note: Uint8Array/BodyInit lib-strictness mismatch, see PLAN.md
			signal,
			prepareInit: () => {
