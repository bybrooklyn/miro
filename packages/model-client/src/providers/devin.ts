...
import {
	...
	StopReason,
} from "@miro/model-catalog/discovery/devin-proto";
import { create, fromBinary, toBinary } from "@miro/model-catalog/discovery/protobuf";
import { calculateCost } from "@miro/model-catalog/models";
import { DEVIN_DEFAULT_BASE_URL, devinCliMetadata } from "@miro/model-catalog/wire/devin";
import { decodeDevinUnaryMessage } from "@miro/model-catalog/wire/devin-proto";
import { logger, parseStreamingJson, parseStreamingJsonThrottled } from "@miro/agent-sys";
...
async function fetchDevinAuthMetadata(
	...
	const response = await fetchImpl(`${baseUrl}${DEVIN_AUTH_PATH}`, {
		...
		body: toBinary(GetUserJwtRequestSchema, request) as BodyInit, // Vendoring note: Uint8Array/BodyInit lib mismatch
		signal,
	});
...
async function assignDevinModel(
	...
	const response = await fetchImpl(`${baseUrl}${DEVIN_ASSIGN_MODEL_PATH}`, {
		...
		body: toBinary(AssignModelRequestSchema, request) as BodyInit, // Vendoring note: Uint8Array/BodyInit lib mismatch
		signal,
	});
	const payload = new Uint8Array(await response.arrayBuffer());
	if (!response.ok) {
