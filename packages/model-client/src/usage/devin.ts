...
import { create, toBinary } from "@miro/model-catalog/discovery/protobuf";
import { DEVIN_DEFAULT_BASE_URL, devinCliMetadata, normalizeDevinSessionToken } from "@miro/model-catalog/wire/devin";
...
async function fetchDevinUsage(params: UsageFetchParams, ctx: UsageFetchContext): Promise<UsageReport | null> {
	...
	try {
		...
		const response = await ctx.fetch(`${baseUrl}${GET_USER_STATUS_PATH}`, {
			...
			body: toBinary(GetUserStatusRequestSchema, request) as BodyInit, // Vendoring note: Uint8Array/BodyInit lib mismatch
