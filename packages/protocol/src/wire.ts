// The wire protocol itself: every message and event, the framing, and the ALPN. Deliberately free of
// node:fs / node:os / node:path and of `process`, so a browser can bundle it - the web client imports
// `@miro/protocol/wire` and gets the contract without the daemon's filesystem layout. Found live: a
// real browser refused the bundle with "process is not defined" when this lived beside the path
// resolution below. Documented in docs/protocol.md.
 ...
 * IROH_ALPN suffix with it. Clients may send it, and should tolerate events they do not know. */
