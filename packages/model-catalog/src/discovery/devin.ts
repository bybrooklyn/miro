import { logger } from "@miro/agent-sys";
...
export async function fetchDevinModels(
	...
	try {
		...
		// Vendoring note: same TS lib-strictness mismatch as ptree.ts's Blob fix - Uint8Array's
		// ArrayBufferLike backing vs BodyInit's ArrayBuffer-only ArrayBufferView. fetch accepts a
		// Uint8Array body unconditionally at runtime.
		const response = await fetchImpl(requestUrl, { method: "POST", headers, body: body as BodyInit, signal });
		if (!response.ok) {
			return null;
		}
