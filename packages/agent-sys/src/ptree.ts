...
export class ChildProcess<In extends InMask = InMask> {
	...
	async blob(): Promise<Blob> {
		// Vendoring note: Uint8Array<ArrayBufferLike> vs BlobPart's ArrayBufferView<ArrayBuffer> is a
		// TS lib-version strictness mismatch (SharedArrayBuffer-vs-ArrayBuffer backing), not a real
		// runtime issue - Bun's Blob constructor accepts a Uint8Array unconditionally.
		return new Blob([await this.#readOutputBytes(true)] as BlobPart[]);
	}
