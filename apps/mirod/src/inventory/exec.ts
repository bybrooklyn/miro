...
export async function run(cmd: string, args: string[], opts: { timeoutMs?: number; env?: Record<string, string> } = {}): Promise<string> {
  // `env`, when given, is MERGED onto the daemon's env, never a replacement - a bare `env` would
  // drop PATH/HOME and break git/ssh. Only the backup module's git calls use it (GIT_TERMINAL_PROMPT
  // off so an auth failure fails fast instead of hanging, GIT_SSH_COMMAND for the deploy key).
  const env = opts.env ? { ...process.env, ...opts.env } : undefined;
  const { stdout } = await execFileAsync(cmd, args, { maxBuffer: 16 * 1024 * 1024, timeout: opts.timeoutMs ?? DEFAULT_RUN_TIMEOUT_MS, env });
  return stdout;
}
...
export function runPrivileged(args: string[], opts: { timeoutMs?: number; env?: Record<string, string> } = {}): Promise<string> {
