...
test("context block names the research providers once the capability layer is configured", async () => {
  const { configureCapabilities, resetCapabilities } = await import("../capabilities");
  ...
  try {
    ...
    resetCapabilities(); // the module singleton would otherwise leak into every later test file
  }
});
